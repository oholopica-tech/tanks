...
// ===== РЕАЛЬНАЯ ВАЛЮТА И ОПЫТ =====
let userCredits = 0;
let userGold = 0;
let userPremiumMinutes = 0;
let userExperience = 0;
let userFreeExperience = 0;
let currentTankXP = 0;
let currentTank = 'minotauro';
...
// ===== КОМАНДЫ АДМИНА (ЧЕРЕЗ КОНСОЛЬ) =====
window.adminCommands = {
    giveCredits: function(uid, amount) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !amount || amount <= 0) {
            console.log('❌ Использование: giveCredits("uid_пользователя", количество)');
            return;
        }
        database.ref('users/' + uid + '/credits').transaction((current) => {
            return (current || 0) + amount;
        }).then(() => {
            console.log(`✅ Выдано ${amount} кредитов пользователю ${uid}`);
    ...
    giveGold: function(uid, amount) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !amount || amount <= 0) {
            console.log('❌ Использование: giveGold("uid_пользователя", количество)');
            return;
        }
        database.ref('users/' + uid + '/gold').transaction((current) => {
            return (current || 0) + amount;
        }).then(() => {
            console.log(`✅ Выдано ${amount} золота пользователю ${uid}`);
            if (currentUser && currentUser.uid === uid) {
                userGold += amount;
                updateCurrencyDisplay();
            }
    ...
    givePremium: function(uid, minutes) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !minutes || minutes <= 0) {
            console.log('❌ Использование: givePremium("uid_пользователя", минуты)');
            return;
        }
        database.ref('users/' + uid + '/premiumMinutes').transaction((current) => {
            return (current || 0) + minutes;
        }).then(() => {
            console.log(`✅ Выдано ${minutes} минут премиума пользователю ${uid}`);
            if (currentUser && currentUser.uid === uid) {
                userPremiumMinutes += minutes;
                updateCurrencyDisplay();
            }
        }).catch(err => console.log('❌ Ошибка:', err));
    },

    giveExperience: function(uid, amount) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !amount || amount <= 0) {
            console.log('❌ Использование: giveExperience("uid_пользователя", количество)');
            return;
        }
        database.ref('users/' + uid + '/experience').transaction((current) => {
            return (current || 0) + amount;
        }).then(() => {
            console.log(`✅ Выдано ${amount} опыта пользователю ${uid}`);
            if (currentUser && currentUser.uid === uid) {
    ...
    giveFreeXP: function(uid, amount) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !amount || amount <= 0) {
            console.log('❌ Использование: giveFreeXP("uid_пользователя", количество)');
            return;
        }
        database.ref('users/' + uid + '/freeExperience').transaction((current) => {
            return (current || 0) + amount;
        }).then(() => {
            console.log(`✅ Выдано ${amount} свободного опыта пользователю ${uid}`);
            if (currentUser && currentUser.uid === uid) {
                userFreeExperience += amount;
                updateCurrencyDisplay();
            }
        }).catch(err => console.log('❌ Ошибка:', err));
    },

    giveTankXP: function(uid, amount) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!uid || !amount || amount <= 0) {
            console.log('❌ Использование: giveTankXP("uid_пользователя", количество)');
            return;
        }
        const tankRef = 'users/' + uid + '/tanks/' + currentTank + '/xp';
        database.ref(tankRef).transaction((current) => {
            return (current || 0) + amount;
        }).then(() => {
            console.log(`✅ Добавлено ${amount} опыта на танк ${currentTank}`);
            if (currentUser && currentUser.uid === uid) {
                currentTankXP += amount;
                updateCurrencyDisplay();
                checkTankResearch();
            }
        }).catch(err => console.log('❌ Ошибка:', err));
    ...
    help: function() {
        console.log('===== КОМАНДЫ АДМИНА =====');
        console.log('giveCredits("uid", количество) - Выдать кредиты');
        console.log('giveGold("uid", количество) - Выдать золото');
        console.log('givePremium("uid", минуты) - Выдать премиум аккаунт');
        console.log('giveExperience("uid", количество) - Выдать опыт для прокачки');
        console.log('giveFreeXP("uid", количество) - Выдать свободный опыт');
        console.log('giveTankXP("uid", количество) - Добавить опыт на текущий танк');
        console.log('getUserData("uid") - Получить данные пользователя');
        console.log('researchModule("модуль") - Исследовать модуль');
        console.log('==============================');
    },

    getUserData: function(uid) {
        if (!uid) {
            console.log('❌ Укажите UID пользователя');
            return;
        }
    ...
    researchModule: function(moduleName) {
        if (!currentUser) {
            console.log('❌ Вы не авторизованы');
            return;
        }
        if (!moduleName) {
            console.log('❌ Укажите название модуля');
            return;
        }
        const moduleCost = getModuleCost(moduleName);
        if (userExperience < moduleCost) {
            console.log(`❌ Недостаточно опыта! Нужно: ${moduleCost}, у вас: ${userExperience}`);
            return;
        }
        userExperience -= moduleCost;
        database.ref('users/' + currentUser.uid + '/experience').set(userExperience);
        database.ref('users/' + currentUser.uid + '/researchedModules/' + moduleName).set(true);
        console.log(`✅ Модуль "${moduleName}" исследован! Потрачено: ${moduleCost} опыта`);
        updateCurrencyDisplay();
        showNotification(`🔬 Модуль "${moduleName}" исследован!`);
    }
};

// ===== СИСТЕМА ОПЫТА (КАК В РЕАЛЬНЫХ ТАНКАХ) =====
function getModuleCost(moduleName) {
    ...
    return costs[moduleName] || 0;
}

function getTankResearchCost(tankName) {
    const costs = {
        'minotauro': 0,
        'fv205b': 35000,
        'ho_ri': 42000,
    ...
    return costs[tankName] || 0;
}

function checkTankResearch() {
    const tanks = ['minotauro', 'fv205b', 'ho_ri', 'wz_70', 't110e3', 'obj268'];
    const currentIndex = tanks.indexOf(currentTank);
    if (currentIndex < tanks.length - 1) {
        const nextTank = tanks[currentIndex + 1];
        const cost = getTankResearchCost(nextTank);
        const tankXP = currentTankXP;
        if (tankXP >= cost) {
            const button = document.getElementById('researchNextTank');
            if (button) {
                button.style.display = 'block';
                button.textContent = `🔬 Исследовать ${nextTank.toUpperCase()} (${cost} опыта)`;
                button.onclick = () => researchNextTank(nextTank, cost);
            }
        }
    }
}

function researchNextTank(tankName, cost) {
    if (currentTankXP < cost) {
        showNotification(`❌ Недостаточно опыта на танке! Нужно: ${cost}`);
        return;
    }
    
    currentTankXP -= cost;
    database.ref('users/' + currentUser.uid + '/experience').transaction((current) => {
        return (current || 0) + cost;
    });
    database.ref('users/' + currentUser.uid + '/tanks/' + tankName + '/researched').set(true);
    database.ref('users/' + currentUser.uid + '/tanks/' + currentTank + '/xp').set(currentTankXP);
    
    currentTank = tankName;
    currentTankXP = 0;
    
    updateCurrencyDisplay();
    showNotification(`🎉 Исследован новый танк: ${tankName.toUpperCase()}!`);
...
function updateTankCarousel() {
    const cards = document.querySelectorAll('.t-card');
    const tankNames = ['minotauro', 'fv205b', 'ho_ri', 'wz_70'];
    cards.forEach((card, index) => {
        if (index < tankNames.length) {
            const name = card.querySelector('.t-name');
            if (name) name.textContent = tankNames[index].toUpperCase();
        }
    });
}
...
function loadUserData(uid) {
    database.ref('users/' + uid).once('value').then(function(snapshot) {
        ...
        if (data) {
            ...
            userPremiumMinutes = data.premiumMinutes || 0;
            userExperience = data.experience || 0;
            userFreeExperience = data.freeExperience || 0;
            
            if (data.tanks) {
                const tankData = data.tanks[currentTank];
                if (tankData) {
                    currentTankXP = tankData.xp || 0;
                }
            }
...
function updateCurrencyDisplay() {
    ...
    const premiumEl = document.getElementById('userPremium');
    
    if (creditsEl) creditsEl.textContent = userCredits.toLocaleString();
    if (goldEl) goldEl.textContent = userGold.toLocaleString();
    if (premiumEl) premiumEl.textContent = formatPremiumTime(userPremiumMinutes);
}

function updateXPDisplay() {
    const expEl = document.getElementById('userExperience');
    const freeExpEl = document.getElementById('userFreeXP');
    const tankExpEl = document.getElementById('currentTankXP');
    
    if (expEl) expEl.textContent = userExperience.toLocaleString();
    if (freeExpEl) freeExpEl.textContent = userFreeExperience.toLocaleString();
    if (tankExpEl) tankExpEl.textContent = currentTankXP.toLocaleString();
}

function formatPremiumTime(minutes) {
    if (minutes <= 0) return 'Нет премиума';
...
function startPremiumTimer() {
    setInterval(() => {
        if (userPremiumMinutes > 0) {
            userPremiumMinutes--;
            updateCurrencyDisplay();
            if (currentUser) {
                database.ref('users/' + currentUser.uid + '/premiumMinutes').set(userPremiumMinutes);
            }
        }
    }, 60000);
}
        ...
        .then((userCredential) => {
            ...
            return database.ref('users/' + user.uid).set({
                ...
                stats: {
                    ...
                    avgDamage: 0, rating: 0, destroyed: 0,
                    survival: 0, accuracy: 0
                }
            }).then(() => {
                currentUser = user;
                currentUserData = { 
                    ...
                    freeExperience: 0,
                    tanks: {
                        minotauro: { xp: 0, researched: true }
                    },
                    stats: { 
                        battles: 0, wins: 0, winrate: 0,
                        mastery: 0, class1: 0, class2: 0, class3: 0
                ...
                userCredits = 1000;
                userGold = 500;
                userPremiumMinutes = 0;
                userExperience = 0;
                userFreeExperience = 0;
                currentTankXP = 0;
                errorEl.style.display = 'none';
                showGameWithPostLoading();
            });
...
function updateStats(won, damage, destroyed) {
    ...
    statsRef.once('value').then(function(snapshot) {
        ...
        return statsRef.set(stats).then(() => {
            ...
            const xpEarned = Math.floor(100 + Math.random() * 400 + (won ? 150 : 0) + damage / 10);
            const freeXPEarned = Math.floor(xpEarned * 0.05);
            
            userExperience += xpEarned;
            currentTankXP += xpEarned;
            userFreeExperience += freeXPEarned;
            
            database.ref('users/' + currentUser.uid + '/experience').set(userExperience);
            database.ref('users/' + currentUser.uid + '/freeExperience').set(userFreeExperience);
            database.ref('users/' + currentUser.uid + '/tanks/' + currentTank + '/xp').set(currentTankXP);
            
            updateCurrencyDisplay();
            updateXPDisplay();
            
            showNotification(`⭐ Заработано ${xpEarned} опыта на ${currentTank.toUpperCase()}`);
            checkTankResearch();
        });
    });
}
...
// ===== БЛОКИРОВКА КЛИКОВ В МЕНЮ =====
document.addEventListener('click', function(e) {
    const settings = document.getElementById('settingsPanel');
    const news = document.getElementById('newsPanel');
    const chat = document.getElementById('chatFullscreen');
    const stats = document.getElementById('statsFullscreen');
    
    const isMenuOpen = (settings && settings.classList.contains('active')) ||
                      (news && news.classList.contains('active')) ||
                      (chat && chat.classList.contains('active')) ||
                      (stats && stats.classList.contains('active'));
    
    if (isMenuOpen) {
        const isClickInsideMenu = (settings && settings.contains(e.target)) ||
                                  (news && news.contains(e.target)) ||
                                  (chat && chat.contains(e.target)) ||
                                  (stats && stats.contains(e.target));
...
function activatePromoCode() {
    ...
    const validCodes = {
        ...
        'BLITZ2026': '⚡ 1000 золота + 5000 опыта',
        'WOTBLITZ': '🎯 750 золота + 3000 опыта',
        'HELLO2026': '👋 200 золота + 1000 опыта',
        'GOLD2026': '💰 1500 золота',
        'PROMO2026': '🎁 Премиум 7 дней',
        'VIP2026': '👑 VIP статус на 30 дней',
        'STAR2026': '⭐ 10000 опыта + 1000 золота'
    };
    ...
    if (validCodes[code]) {
        ...
        if (code === 'TANK2026' || code === 'PROMO2026' || code === 'VIP2026') {
            let minutes = 0;
            if (code === 'TANK2026') minutes = 4320;
            else if (code === 'PROMO2026') minutes = 10080;
            else if (code === 'VIP2026') minutes = 43200;
            userPremiumMinutes += minutes;
            database.ref('users/' + currentUser.uid + '/premiumMinutes').set(userPremiumMinutes);
        }
        
        if (code === 'BLITZ2026' || code === 'STAR2026' || code === 'WOTBLITZ') {
            let xp = 0;
            if (code === 'BLITZ2026') xp = 5000;
            else if (code === 'STAR2026') xp = 10000;
            else if (code === 'WOTBLITZ') xp = 3000;
            userExperience += xp;
            database.ref('users/' + currentUser.uid + '/experience').set(userExperience);
        }
        ...
        if (code === 'GOLD2026') {
            userGold += 1500;
            database.ref('users/' + currentUser.uid + '/gold').set(userGold);
        }
...
function startBattle() {
    ...
    const settingsOpen = document.getElementById('settingsPanel').classList.contains('active');
    const newsOpen = document.getElementById('newsPanel').classList.contains('active');
    
    if (chatOpen || statsOpen || settingsOpen || newsOpen) {
        showNotification('❌ Закройте все меню перед боем');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('statsBackBtn').addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        toggleStatsFullscreen();
    });
...
console.log('💰 Реальная валюта: кредиты и золото');
console.log('⭐ Опыт для прокачки как в реальных танках');
console.log('🛠️ Команды админа в консоли: adminCommands.help()');