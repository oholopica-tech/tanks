...
function addEnhancedBirds() {
    for (let i = 0; i < 20; i++) {
        ...
        // Тело птицы
        const bodyGeo = new THREE.ConeGeometry(0.15, 0.35, 6);
        ...
        wingLeft.position.set(0, 0.06, 0);
        ...
        wingRight.position.set(0, -0.06, 0);
        wingRight.rotation.z = 0.2;
        wingRight.rotation.x = 0.3;
        birdGroup.add(wingRight);
        
        // Хвост
        const tailGeo = new THREE.PlaneGeometry(0.12, 0.08);
        ...
        birdGroup.userData = {
            ...
            flapSpeed: 0.005 + Math.random() * 0.008,
    ...
    function animateBirdsEnhanced() {
        ...
        birds.forEach(bird => {
            ...
            bird.position.y = bird.userData.height + Math.sin(Date.now() * 0.001 + bird.userData.wingPhase) * 0.8;
            bird.rotation.y = -bird.userData.angle + Math.PI / 2;
            bird.rotation.z = Math.sin(Date.now() * 0.001 + bird.userData.wingPhase) * 0.05;
            
            const wings = bird.children.filter(c => c.geometry && c.geometry.type === 'PlaneGeometry' && c.position.x === 0);
            wings.forEach((wing, index) => {
                const phase = index === 0 ? 1 : -1;
                const flapAngle = Math.sin(Date.now() * bird.userData.flapSpeed * 100 + bird.userData.wingPhase) * 0.8 * phase;
                wing.rotation.z = flapAngle;
                if (index === 0) {
                    wing.rotation.x = -0.3 + Math.sin(Date.now() * bird.userData.flapSpeed * 80 + bird.userData.wingPhase) * 0.1;
                } else {
                    wing.rotation.x = 0.3 + Math.sin(Date.now() * bird.userData.flapSpeed * 80 + bird.userData.wingPhase) * 0.1;
                }
            });
